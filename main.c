...
void eval(char *cmdline) {
  ...
  if (argv[0] == NULL)
    ...
    //!!!try combining the sigpromask functioncall here before the if/else
    if (bg){
      sigprocmask(SIG_UNBLOCK, &mask, NULL);
...
int builtin_cmd(char **argv)
{
  //note adding the extra qualifier number for strncmp
  //verifies it actually checks that number of char letters
